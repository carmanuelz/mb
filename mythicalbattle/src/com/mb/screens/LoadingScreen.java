...
public class LoadingScreen extends AbstractScreen{
	...
	public void render(float delta) {
		...
		if (game.mNativeFunctions.getPercent()==100) { // Load some, will return true if done loading
            if (Gdx.input.isTouched()) { // If the screen is touched after the game is done loading, go to the main menu screen
                game.setScreen(new StartGameScreen(game,new DataStartGame()));
            }
        }
        ...
        batch.begin();
        batch.end();
        fill_mid.setWidth(796*factorw*(game.mNativeFunctions.getPercent()/100));
        fill_mid.invalidate();
